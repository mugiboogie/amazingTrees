...
public class SpellWeaponController : MonoBehaviour
{
    ...
    void Update()
    {
        float setDissolve = (Time.time > playerAttack.spellWeaponVisibleTime) ? 0f : 1f;
        dissolveValue = Mathf.Lerp(dissolveValue, setDissolve, 10f * Time.deltaTime);

        renderer.material.SetFloat("_Progress", dissolveValue);

        if ((Time.time > playerAttack.spellWeaponVisibleTime) && (!playedOff))
        {
        ...
        else if ((Time.time < playerAttack.spellWeaponVisibleTime) && (!playedOn))
        {
